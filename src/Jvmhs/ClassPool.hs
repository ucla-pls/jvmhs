...
getClass cn = getConst <$> alterClass (Const) cn

-- | Set a class on the class pool.
setClass :: MonadClassPool m => ClassName -> Maybe Class -> m ()
setClass cn mc = join $ runIdentity <$> alterClass (\_ -> Identity mc) cn

-- | Modify a class on the class pool.
modifyClass
  :: MonadClassPool m => ClassName -> (Maybe Class -> Maybe Class) -> m ()
modifyClass cn fn = join $ runIdentity <$> alterClass (Identity . fn) cn

-- | Map classes, map over the classes in the class pool
modifyClasses :: MonadClassPool m => (Class -> Maybe Class) -> m ()
modifyClasses fn = runIdentityT $ alterClasses (IdentityT . return . fn)
...
putClass cls = setClass (cls ^. className) (Just cls)

-- | Deletes a class in the class pool.
deleteClass :: MonadClassPool m => ClassName -> m ()
deleteClass cn = setClass cn Nothing
...
onlyClasses' = restrictTo

-- | Get all the classes from the class pool
allClasses :: MonadClassPool m => m [Class]
allClasses = mapClasses id

-- | Get all the classes from the class pool
collectClasses
  :: forall r m . (Monoid r, MonadClassPool m) => (Class -> r) -> m r
collectClasses f =
  execWriterT . alterClasses $ \c -> tell (f c) >> return (Just c)

-- | Get all the classes from the class pool
mapClasses :: MonadClassPool m => (Class -> r) -> m [r]
mapClasses f = flip appEndo [] <$> collectClasses (\c -> Endo (f c :))

-- | Modify all the classes but keep side-effects
modifyClassesM :: MonadClassPool m => (Class -> m (Maybe Class)) -> m ()
modifyClassesM fn = runIdentityT $ alterClasses (lift . fn)

-- | Destroy all classes in the class pool, but not before loading all of them
destroyClassesM :: MonadClassPool m => (Class -> m ()) -> m ()
destroyClassesM fn = modifyClassesM (fn >=> const (return Nothing))

-- | A pool action can be used as part of a lens to access a class.
-- >>> "java.lang.Object" ^! pool.className
-- "java.lang.Object"
pool :: MonadClassPool m => Action m ClassName (Maybe Class)
pool = act getClass

-- | A specialized pool action that returns the name of the class if
-- it does not exist in the ClassPool
pool' :: MonadClassPool m => Action m ClassName (Either ClassName Class)
pool' = act (\cn -> maybe (Left cn) (Right) <$> getClass cn)

-- | Get all the classes from the class pool
getClasses :: (Foldable t, MonadClassPool m) => t ClassName -> m [Class]
getClasses = (^!! folded . pool . _Just)
...
loadClasses
  :: (MonadClassPool m, MonadClassReader r m) => m [ClassPoolReadError]
loadClasses = do
  r          <- ask
...
loadClassesFromReader = runClassReaderT loadClasses

-- | Saves all the classes in the pool to the filepath.
saveAllClasses :: (MonadClassPool m, MonadIO m) => FilePath -> m ()
...
-- @
saveClass :: (MonadClassPool m, MonadIO m) => FilePath -> ClassName -> m ()
saveClass fp cn = do
  saveClasses fp [cn]


getMethod :: (MonadClassPool m) => AbsMethodId -> m (Maybe Method)
getMethod mid = do
  cls <- getClass (mid ^. className)
  return $ cls ^? _Just . classMethod (mid ^. methodId) . _Just
...
loadClassPoolState = runClassPoolTWithReader return

-- | Save class pool state to a file-path
saveClassPoolState :: FilePath -> ClassPoolState -> IO ()
saveClassPoolState fp = fmap fst . runClassPoolT (saveAllClasses fp)
...
--   fmap put . M.traverseMaybeWithKey (\_ cls -> f cls) <$> get
  ...
  allClassNames = M.keys <$> get

  cplocal m = do
    cp <- get
    lift $ fst <$> runClassPoolT m cp

  restrictTo s = modify (flip M.intersection (S.toMap s))

type ClassPool = ClassPoolT Identity

-- | Run a `ClassPoolT` given a class pool
runClassPoolT :: ClassPoolT m a -> ClassPoolState -> m (a, ClassPoolState)
runClassPoolT = runStateT . runClassPoolT'

-- | Run a `ClassPoolT` given a class pool
runClassPool :: ClassPool a -> ClassPoolState -> (a, ClassPoolState)
...
instance (MonadReader r m) => MonadReader r (CachedClassPoolT r' m) where
  ...
  local f m = CachedClassPoolT . StateT $ \s ->
...
traverseState
  ...
  -> ClassState
  -> f (Maybe ClassState)
traverseState fn x = fmap CSPure <$> case x of
...
streamClasses
  :: (ClassReader r, MonadIO m) => (Class -> m ()) -> CachedClassPoolT r m ()
streamClasses fn = do
...
instance (ClassReader r, MonadIO m) => MonadClassPool (CachedClassPoolT r m) where
  ...
  alterClass f n = do
    ...
    return (fmap put m)
   where
    alterer opt = helper (Compose . fmap f . tryEnsureClass n opt)

    helper fn = maybe (fmap CSPure <$> fn Nothing) (traverseState fn)
  ...
  alterClasses f = do
    ...
    lift get
      ...
      .   M.traverseWithKey
            (\n cls -> traverseState
              (lift . tryEnsureClass n opt >=> maybe (return Nothing) f)
              cls
            )
      >>= lift
      .   put
  ...
  saveClasses fp cns = do
   ...
   where
    ...
    helper (Just (CSPure cs)) =
      let bs = serializeClass cs in (Just bs, Just (CSSaved cs bs))
    helper this@(Just (CSSaved _ bs)) = (Just bs, this)
    helper (     Just CSUnread      ) = error "Should have been read"

  cplocal (CachedClassPoolT m) = do
    cp <- get
    r  <- CachedClassPoolT ask
    lift $ fst <$> (flip runReaderT r . flip runStateT cp $ m)
...
runCachedClassPool = runCachedClassPoolT
...
alterF
  ...
  -> M.HashMap k v
  -> f (M.HashMap k v)
alterF f k m = flip fmap (f (M.lookup k m)) $ \case
