...
instance Inspectable a => Inspectable (InClass a) where
  classNames g (InClass t ci) = InClass <$> g t <*> classNames g ci

instance Inspectable AbsFieldId where
  classNames g (AbsFieldId a) = AbsFieldId <$> classNames g a

instance Inspectable FieldId where
  classNames g (FieldId d) = B.FieldId <$> classNames g d

instance Inspectable MethodId where
  classNames g (MethodId d) = B.MethodId <$> classNames g d

instance Inspectable a => Inspectable (B.NameAndType a) where
  classNames g (B.NameAndType n d) = B.NameAndType n <$> classNames g d
...
instance Inspectable FieldDescriptor where
  classNames g fd =
    B.FieldDescriptor <$> classNames g (B.fieldDescriptorType fd)
...
instance Inspectable FieldSignature where
  classNames g (FieldSignature ref) = FieldSignature <$> classNames g ref
...
instance Inspectable ClassType where
  classNames g t = case t of
    ...
    InnerClassType _ cn ta ->
      InnerClassType
        <$> (snd . Text.breakOnEnd "$" . B.classNameAsText <$> g
              (getClassName t)
            )
        <*> classNames g cn
   ...
   where
    ...
    getClassName (InnerClassType i oc _) =
      either error id . deserialize $ Text.intercalate
        "$"
        [B.classNameAsText (getClassName oc), i]

instance Inspectable TypeArgument where
  classNames g (TypeArgument i p) = TypeArgument i <$> classNames g p
