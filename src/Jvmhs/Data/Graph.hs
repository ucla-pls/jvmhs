...
import           Data.Graph.Inductive.Dot       ( fglToDot
                                                , fglToDotGeneric
                                                , showDot
                                                )
...
graphContexts fn gr =
  (innerGraph . ifolding
      (map (\(n, _) -> (n, F.context (gr ^. innerGraph) n)) . F.labNodes)
    )
    fn
    gr
...
partition gr =
  ...
  where asLabels = toListOf (to (IS.toList) . folded . toLabel gr . _Just)
...
partition' graph = catMaybes . V.toList $ cv
 where
  ...
  getEdges = IS.fromList . map ((vMap IM.!) . view _2) . F.out gr
...
forwardRemove gr f = (closure, remove gr closure)
  where closure = close gr (toList f)
...
shrink gr vs = remove gr revset
  where revset = S.setOf grNodes gr `S.difference` S.setOf folded vs
