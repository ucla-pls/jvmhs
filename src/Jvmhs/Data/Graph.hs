...
grNode i = innerGraph . to (`F.lab` i)
...
graphContexts fn gr =
  ( innerGraph
      . ifolding
        (fmap (\(n, _) -> (n, F.context (gr ^. innerGraph) n)) . F.labNodes)
  )
    fn
    gr
...
mkGraph ::
  forall f t v e.
  (Foldable f, Foldable t, Ord v, Ord e) =>
  f v ->
  t (v, v, e) ->
  Graph v e
mkGraph nodes edges = Graph (F.mkGraph nodes_ edges_) nodemap
 where
  nodes_ = itoListOf folded nodes
  nodemap = M.fromList $ swap <$> nodes_
  edges_ = edges ^.. folded . to nLookup . _Just
  nLookup (v1, v2, e) =
    (,,) <$> (nodemap M.!? v1) <*> (nodemap M.!? v2) <*> pure e

-- | Create a graph from a collection of edges.
mkGraphFromEdges :: (Foldable f, Ord v, Ord e) => f (v, v, e) -> Graph v e
mkGraphFromEdges edges =
  mkGraph (edges ^. folded . (_1 <> _2) . to S.singleton) edges

-- | Check if a foldable collection of items is closed in the graph
isClosedIn :: (Foldable f, Ord v) => f v -> Graph v e -> Bool
isClosedIn vs gr = closure == input
 where
  input = L.sort $ vs ^.. folded . fromLabel gr . _Just
  closure = L.sort $ dfs input (gr ^. innerGraph)

-- | Given a list of items close under them.
close :: (Foldable f, Ord v) => Graph v e -> f v -> [v]
close gr vs = closure ^.. folded . toLabel gr . _Just
 where
...
collapse :: (Foldable f, Ord v) => Graph v e -> f v -> [v]
collapse gr vs = left ^.. to IS.toList . folded . toLabel gr . _Just
 where
  input = vs ^.. folded . fromLabel gr . _Just
  inputSet = IS.fromList input
  missed = IS.fromList (dfs input (gr ^. innerGraph)) `IS.difference` inputSet
  left =
    inputSet
      `IS.difference` IS.fromList (rdfs (IS.toList missed) (gr ^. innerGraph))

revclose :: (Ord v) => Graph v e -> [v] -> [v]
revclose gr vs = closure ^.. folded . toLabel gr . _Just
 where
  input = vs ^.. folded . fromLabel gr . _Just
  closure = rdfs input (gr ^. innerGraph)

-- | Output the graph as a dot graph string
graphToDot :: (Show v, Show e) => Graph v e -> String
graphToDot = showDot . fglToDot . _innerGraph

-- | Output the graph as a dot graph string, with string methods
graphToDot' :: Graph v e -> (v -> String) -> (e -> String) -> String
graphToDot' gr fv fe = showDot $ fglToDotGeneric (_innerGraph gr) fv fe id

-- | Create a graph of strongly connected components.
sccGraph :: (Ord v) => Graph v e -> (NodeMap v, Graph [v] ())
sccGraph gr = (nodemap', Graph gr' nodemap)
 where
  ...
  unfoldN (n, l) = fmap (,n) l

revReachable :: Ord v => Graph v e -> v -> [v]
revReachable gr@(Graph g _) v = case v ^. fromLabel gr of
  Just x -> rdfs [x] g ^.. folded . toLabel gr . _Just
  Nothing -> []

reverseFold :: Ord v => Graph v e -> (v -> [(v, e, m)] -> m) -> v -> Maybe m
reverseFold gr@(Graph g _) f = preview (fromLabel gr . _Just . to go)
 where
  go vid =
    f
      (gr ^?! grNode vid . _Just)
      [(gr ^?! grNode vid' . _Just, e, go vid') | (vid', e) <- F.lpre g vid]
...
partition gr =
  fmap (each %~ asLabels) . L.sortOn (IS.size . snd) . partition' $ gr
 where
  asLabels = toListOf (to IS.toList . folded . toLabel gr . _Just)

closures :: Graph v e -> [IS.IntSet]
closures = fmap snd . partition'
...
partition' graph = catMaybes . V.toList $ cv
 where
  gr = graph ^. innerGraph
  sccs = scc gr
  iscc = zip [0 ..] sccs
  vMap = IM.fromList . concatMap (\(i, xs) -> fmap (,i) xs) $ iscc

  edges = fmap (\(i, ls) -> IS.unions . fmap (IS.delete i . getEdges) $ ls) iscc

  cv = V.fromList (zipWith getNode sccs edges)

  getEdges = IS.fromList . fmap ((vMap IM.!) . view _2) . F.out gr

  getNode :: [Int] -> IS.IntSet -> Maybe (IS.IntSet, IS.IntSet)
  getNode s t = do
    let Just before = sequence [(cv V.! b) ^? _Just . _2 | b <- IS.toList t]
        s' = IS.fromList s
...
-}
forwardRemove :: (Foldable f, Ord v) => Graph v e -> f v -> ([v], Graph v e)
forwardRemove gr f = (closure, remove gr closure)
 where
  closure = close gr (toList f)

remove :: (Foldable f, Ord v) => Graph v e -> f v -> Graph v e
remove gr vs =
  gr
    & innerGraph
    %~ F.delNodes (vs ^.. folded . fromLabel gr . _Just)
    & nodeMap
    %~ flip M.withoutKeys (S.setOf folded vs)

shrink :: (Foldable f, Ord v) => Graph v e -> f v -> Graph v e
shrink gr vs = remove gr revset
 where
...
graphFromFile :: FilePath -> IO (Graph Int ())
graphFromFile filepath = withFile filepath ReadMode $ \h -> do
